...
// S키 누르면 다 죽임
void Hack(Character* character, Monster* Mob)
{
	if (nkey[KEY_INPUT_S] == 1)
		Mob->MobHeart = 0;
}
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)   
{
	...
	while (ScreenFlip() == 0 && ProcessMessage() == 0 && ClearDrawScreen() == 0 && UpdateKey() == 0)
	{
		...
		if (map_x == boss_room_x && map_y == boss_room_y)										//	만약 보스 스테이지에 도달했다면
		{
			...
			//	게임클리어가 정상 출력되는지 확인하기 위한 핵 ( f키를 누르면 보스 사망 )
			if (nkey[KEY_INPUT_F] >= 1)
				boss.BossExist = 2;
		...
		for (i = 0; i < 5; i++)
			Hack(&character, &monster[i]);

		//	글씨 각종 정보 출력
		DrawFormatString(750, 130, GetColor(255, 255, 255), "Bullet : %d", remain_bullet);                      //  남은 총알 개수 출력
		//DrawFormatString(900, 150, GetColor(255, 255, 255), "x : %d, y : %d", character.CharX, character.CharY);	//	최종버전에서는 삭제
		...
		//DrawFormatString(300, 110, GetColor(255, 255, 255), "Character His x : %d, y : %d", character_history_x, character_history_y);	//	최종버전에서는 삭제
